...
public class IdentifyPerson1 : MonoBehaviour
{
    ...
    private WebCamTexture frontFacingCamera;
    ...
    private void Start()
    {
        ...
        for (int i = 0; i < devices.Length; i++)
        {
            ...
            {
                frontFacingCamera = new WebCamTexture(devices[i].name, Screen.width, Screen.height);
            }
        }
        ...
        // Sets the front facing camera to play.
        frontFacingCamera.Play();
        // Sets the raw image to the front facing camera.
        background.texture = frontFacingCamera;
    ...
    private void Update()
    {
        ...
        float screenRatio = (float)frontFacingCamera.width / (float)frontFacingCamera.height;
        ...
        float scaleY = frontFacingCamera.videoVerticallyMirrored ? -1f : 1f;
        // Sets the Rect transform taking in the new scale for the Y value created above.
        background.rectTransform.localScale = new Vector3(1f, scaleY, 1f);

        int orient = -frontFacingCamera.videoRotationAngle;
